# coding: utf-8
"""
High-throughput magnetic-ordering and SOC workflow for orthorhombic
permanent-magnet screening.

This module extends atomate's magnetic ordering workflow with spin-orbit
coupling calculations along the three orthorhombic principal axes. It is
intended for comparing FM/AFM magnetic orderings and preparing magnetic
anisotropy energy (MAE) data from SOC total-energy differences among [100],
[010], and [001].

Workflow summary:
    1. Enumerate candidate collinear magnetic orderings with pymatgen.
    2. Relax each ordering and run a static calculation.
    3. For FM-derived structures, add SOC calculations along [100], [010],
       and [001].
    4. Store magnetic-ordering metadata and analysis records in the atomate
       database.

Notes:
    - This workflow defaults to the conventional standardized cell because
      orthorhombic MAE is naturally compared along the conventional a, b, and
      c axes.
    - Set standard_cell="primitive" in get_wf() if the SOC branch should use
      the primitive standardized cell instead.
    - num_samples is kept in the public API for compatibility with related
      workflows, but orthorhombic SOC sampling is fixed to the three principal
      directions.
    - nbands_factor controls SOC NBANDS scaling, and kpoints_factor scales SOC
      reciprocal density.
"""
...
def get_saxis_from_crystal_type(crystal_type, num_samples=7):
    """Return SOC SAXIS directions for orthorhombic MAE calculations."""
    unique_saxis = {}

    if crystal_type != "orthorhombic":
        print(f"Warning: crystal type is {crystal_type}. Using orthorhombic principal axes.")
...
def get_standardized_structure_for_soc(structure, standard_cell="conventional", symprec=0.1):
    """Standardize the FM structure before SOC calculations.

    Args:
        structure: pymatgen Structure used for the FM SOC branch.
        standard_cell: "conventional" or "primitive".
    ...
    raise ValueError(
        "standard_cell must be either 'conventional' or 'primitive', "
...
class MagneticOrderingsSOCWF:
    ...
    def get_wf(
        ...
        standard_cell="conventional", crystal_type_override="orthorhombic"
    ):
        """
        ...
        of equivalent symmetry
            ...
            standard_cell: choose "conventional" or "primitive" for the FM
        structure used in SOC calculations
            crystal_type_override: optionally override the detected crystal
        system used to choose SAXIS sampling directions; defaults to
        "orthorhombic" so the workflow always compares [100], [010], and [001]
        ...
        # and static energies are later aggregated by MagneticOrderingsToDB.
        for idx, [ordered_structure, ordered_structure_origin] in enumerate(zip(ordered_structures, 
            ...
            if ordered_structure_origin =='fm':
                ...
                # The three SOC children calculate energies for magnetization
                # along the orthorhombic a, b, and c axes.
                fm_parent = fws[-1]
